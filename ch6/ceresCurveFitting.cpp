...
struct CURVE_FITTING_COST {
  ...
  template<typename T>
  bool operator()(
    const T *const abc, // Model parameters in 3D
    T *residual) const {
...
int main(int argc, char **argv) {
  ...
  // constructing a least-square prob.
  ceres::Problem problem;
  for (int i = 0; i < N; i++) {
    problem.AddResidualBlock(     // Adding error terms to the problem
      // Use automatic differentiation, template parameters: error type, 
      // output dimension, input dimension, the number of dimensions 
      // must be consistent with the previous struct.
      new ceres::AutoDiffCostFunction<CURVE_FITTING_COST, 1, 3>(
        new CURVE_FITTING_COST(x_data[i], y_data[i])
      ),
      nullptr,            // Kernel function, not used here, empty
      abc                 // param. to be estimated
    );
  }
