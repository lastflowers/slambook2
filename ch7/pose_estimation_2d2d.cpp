...
int main(int argc, char **argv) {
  ...
  cout << "Totally found" << matches.size() << "Group matching points" << endl;
  ...
  //-- Verifying epipolar constraints
...
void find_feature_matches(const Mat &img_1, const Mat &img_2,
  ...
  // that is, the distance between the two groups of points that are most similar and least
  ...
  // the match is considered to be incorrect. 
...
void pose_estimation_2d2d(std::vector<KeyPoint> keypoints_1,
                          ...
                          std::vector<DMatch> matches,
                          Mat &R, Mat &t) {
  // Camera reference,TUM Freiburg2
  Mat K = (Mat_<double>(3, 3) << 520.9, 0, 325.1, 0, 521.0, 249.7, 0, 0, 1);

  //-- Converts matching points to the form of vector <Point2f>
  ...
  Point2d principal_point(325.1, 249.7);  //Camera light center, TUM dataset calibration value
  ...
  //-- But in this example, the scene is not flat, so the homography matrix is ​​not significant.
